...
def traverse():
    ...
    #filepath   =  os.getcwd() + "/url_data.pickle"
    list1 = []
    for dirpath, dirs, files in os.walk(filepath):
        for single_file in files:
            if (fnmatch.fnmatch(single_file,'*log') or fnmatch.fnmatch(single_file,'*txt')):
                ...
                properties = os.stat(dirpath1)
                time_modified = os.path.getmtime(dirpath1)
                file_size = properties.st_size
                tuple1 = (dirpath1, content.read(),time_modified, file_size)
                list1.append(tuple1)
