#Program to perform search operation on a dataset - optimized using dictionary part2
...
while(userchoice.lower() == 'yes'):
    ...
    if ("or" in query) and ("and" not in query):
        ...
        time1 = datetime.now() 
        for words in Dict_Words[query[0]]:
            print (" Found at: ", words, data_list[words][:80])
        time2 = datetime.now()
        print("\nExecution Time  :"+str(time2.microsecond-time1.microsecond))
...
Please enter your search querry : sheep

Single word Search as per user request on querry: ['sheep'].......
...
Do you want to perform another search(yes/no) : yes

Please enter your search querry : sheep or flower

OR Search as per user request on querry: ['flower', 'or', 'sheep'].......
...
Please enter your search querry : sheep and flower

AND or And/OR Search as per user request on querry: ['and', 'flower', 'sheep'].......
 Found at:  31 For millions of years flowers have been producing thorns. For millions of years 
...
The observation made is that with usage of dictionary search time got improved significantly,
Since the dictonary created a key value pair with indexes of ocuurance of the word as values and key
this greatly helped the optimising search operation
